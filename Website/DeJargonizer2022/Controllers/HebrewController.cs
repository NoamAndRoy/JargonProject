...
{
    public class HebrewController : Controller
	{
		...
		public ActionResult Index(string ContentTA, HttpPostedFileBase ArticleFU, string timePriodDDL)
		{
			...
			if (timePriodDDL == "2012_2017")
			{
				TextGrading.Lang = Language.Hebrew2012_2017;
			}
			else if (timePriodDDL == "2012_2015")
			{
				TextGrading.Lang = Language.Hebrew2012_2015;
			}
...
Number Of Words:	{numberOfWords}";
	...
	, i_ArticleGradingInfo.CleanedWords.Count.ToString());
		...
		private DocX CreateDocX1(ArticleGradingInfo i_ArticleGradingInfo, string i_Path)
		{
			string templatePath = HttpContext.Server.MapPath(@"~\Content\Assets\templateHeb.docx");
			...
			using (DocX doc = DocX.Load(templatePath))
			{
				...
				foreach (string word in i_ArticleGradingInfo.Words)
				{
					...
					wordParagraph.Alignment = Alignment.right;
					wordParagraph.Direction = Direction.LeftToRight;
				}
